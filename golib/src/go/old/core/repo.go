...
func InitRepoDefaultConfig(repoPath string) error {
	log.Tracef("initRepo() at %s", repoPath)
	if _, err := os.Stat(repoPath); os.IsNotExist(err) {
		log.Warnf("%s does not exist.", repoPath)
		err := os.MkdirAll(repoPath, os.ModePerm)
		if err != nil {
			return err
		}
		log.Tracef("created %s", repoPath)
	}
