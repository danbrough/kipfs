...
"linuxAmd64")
  ...
  SYSROOT=$KONAN_DATA_DIR/dependencies/x86_64-unknown-linux-gnu-gcc-8.3.0-glibc-2.19-kernel-4.9-2/x86_64-unknown-linux-gnu/sysroot
  export PATH=$KONAN_DATA_DIR/dependencies/x86_64-unknown-linux-gnu-gcc-8.3.0-glibc-2.19-kernel-4.9-2/bin:$KONAN_DATA_DIR/dependencies/x86_64-unknown-linux-gnu-gcc-8.3.0-glibc-2.19-kernel-4.9-2/x86_64-unknown-linux-gnu/bin:$PATH
...
if [ "$GOOS" == "linux" ]; then
  export CGO_CFLAGS="-fPIC --sysroot=$SYSROOT -I$SRCDIR/build/native/$PLATFORM/static/ -I$OPENSSL/include "
  export CGO_LDFLAGS="-fPIC --sysroot=$SYSROOT -L$KONAN_DATA_DIR/dependencies/x86_64-unknown-linux-gnu-gcc-8.3.0-glibc-2.19-kernel-4.9-2/lib -L$SYSROOT/lib -L$SYSROOT/usr/lib -L$OPENSSL/lib -lcrypto -lssl  -ldl -lc  "
else
  export PKG_CONFIG_PATH=$SRCDIR/openssl/libs/$PLATFORM/lib/pkgconfig/
  export CGO_CFLAGS="-Wno-deprecated-declarations -fPIC -pthread `pkg-config --cflags libssl ` "
  export CGO_CFLAGS="-pthread  -Wno-deprecated-declarations -fPIC -I/home/dan/workspace/demos/mpp/openssl/libs/windowsAmd64/include -I/usr/x86_64-w64-mingw32/include   "
...
CMD="go build -v -tags=shell,node,openssl   -ldflags '-linkmode external -extldflags "-static"' -buildmode=c-archive -o $LIBNAME  ./libs/ "
go build -v -tags=shell,node,openssl   -ldflags '-linkmode external -extldflags "-static"' -buildmode=c-archive -o $LIBNAME  ./libs/
